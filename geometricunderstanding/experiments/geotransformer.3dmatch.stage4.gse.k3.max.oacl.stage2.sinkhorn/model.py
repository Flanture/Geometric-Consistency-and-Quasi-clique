...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.transformer = GeometricTransformer(
            ...
            cfg.geotransformer.sigma_g,
            cfg.geotransformer.scales,
            cfg.geotransformer.k_neighbors,
    ...
    def forward(self, data_dict):
        ...
        ref_point_to_node_indices, ref_node_masks, ref_node_knn_indices, ref_node_knn_masks = point_to_node_partition(
            ref_points_f, ref_points_c, self.num_points_in_patch
        )
        src_point_to_node_indices, src_node_masks, src_node_knn_indices, src_node_knn_masks = point_to_node_partition(
        ...
        output_dict['ref_padded_points_f'] = ref_padded_points_f
        output_dict['src_padded_points_f'] = src_padded_points_f
        output_dict['ref_node_knn_points'] = ref_node_knn_points
        output_dict['src_node_knn_points'] = src_node_knn_points
        output_dict['ref_point_to_node_indices'] = ref_point_to_node_indices
        output_dict['src_point_to_node_indices'] = src_point_to_node_indices
        ...
        # 10. Output ref_corr_points and src_corr_points's indexes in the ref_points_f and src_points_f
        ref_corr_points_list = ref_corr_points.tolist()
        src_corr_points_list = src_corr_points.tolist()
        ref_points_f_list = ref_points_f.tolist()
        src_points_f_list = src_points_f.tolist()

        # 构建KDTree来加速查找
        ref_tree = KDTree(ref_points_f_list)
        src_tree = KDTree(src_points_f_list)

        # 定义允许的误差范围，例如1e-5
        tolerance = 1e-5

        # 对ref_corr_points查找最近的点索引
        ref_indices = []
        for point in ref_corr_points_list:
            dist, idx = ref_tree.query(point)  # 查找最近点
            if dist < tolerance:
                ref_indices.append(idx)

        # 对src_corr_points查找最近的点索引
        src_indices = []
        for point in src_corr_points_list:
            dist, idx = src_tree.query(point)
            if dist < tolerance:
                src_indices.append(idx)

        output_dict['ref_indices'] = ref_indices
        output_dict['src_indices'] = src_indices

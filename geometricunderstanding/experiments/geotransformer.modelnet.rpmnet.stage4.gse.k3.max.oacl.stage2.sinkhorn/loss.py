...
class Evaluator(nn.Module):
    ...
    @torch.no_grad()
    def evaluate_fmr(self, output_dict, data_dict):
        ...
        src_corr_points = apply_transform(src_corr_points, transform)
        corr_distances = torch.linalg.norm(ref_corr_points - src_corr_points, dim=1)
        inlier_ratio = torch.lt(corr_distances, self.acceptance_radius).float().mean().item()
        
        # Compute Feature Matching Recall (FMR)
        fmr = float(inlier_ratio > self.inlier_ratio_threshold)
    ...
    def forward(self, output_dict, data_dict):
        ...
        fmr = self.evaluate_fmr(output_dict, data_dict)

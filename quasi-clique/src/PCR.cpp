...
float FeatureDistance(const Eigen::VectorXd& feat1, const Eigen::VectorXd& feat2)
{
    return (feat1 - feat2).norm();
}
...
Eigen::MatrixXf Graph_construction(vector<Corre_3DMatch>& correspondence, float resolution, bool sc2, const string &name, const string &descriptor, float inlier_thresh) {
	...
	cmp_score.setZero();
	Corre_3DMatch c1, c2;
	double sum_dis = 0.0, sum_feat_dis = 0.0;
	double sum_dis_sq = 0.0, sum_feat_dis_sq = 0.0;
	double mean_dis, mean_feat_dis, std_dis, std_feat_dis;
	float score, src_dis, des_dis, dis, alpha_dis, src_feat_dis, des_feat_dis, feat_dis, score_feats, score_dis, alpha_feats;

	// 第一遍循环，计算 dis 和 feat_dis 的总和及平方和
	for (int i = 0; i < size; i++) {
		...
		for (int j = i + 1; j < size; j++) {
			...
			src_feat_dis = FeatureDistance(c1.src_features, c2.src_features);
			des_feat_dis = FeatureDistance(c1.des_features, c2.des_features);
			feat_dis = abs(src_feat_dis - des_feat_dis);
			sum_feat_dis += feat_dis;
			sum_feat_dis_sq += feat_dis * feat_dis;
		}
	}

	// 计算均值和标准差
	mean_dis = sum_dis / (size * (size - 1) / 2);
	mean_feat_dis = sum_feat_dis / (size * (size - 1) / 2);
	std_dis = sqrt(sum_dis_sq / (size * (size - 1) / 2) - mean_dis * mean_dis);
	std_feat_dis = sqrt(sum_feat_dis_sq / (size * (size - 1) / 2) - mean_feat_dis * mean_feat_dis);
	...
	else if (name == "3dmatch" || name == "3dlomatch")
	{
		for (int i = 0; i < size; i++)
		{
			...
			for (int j = i + 1; j < size; j++)
			{
				...
				des_feat_dis = FeatureDistance(c1.des_features, c2.des_features);
				feat_dis = abs(src_feat_dis - des_feat_dis);
				// 对 dis 和 feat_dis 进行标准化
				double dis_standard = (dis - mean_dis) / std_dis;
				double feat_dis_standard = (feat_dis - mean_feat_dis) / std_feat_dis;
...
//        0.9 0.95 0.99 0.995 0.999
					...
					alpha_dis = 18.0 * resolution;
					score_dis = exp(- dis * dis /( 2 * alpha_dis * alpha_dis));
					alpha_feats = 23.0;
					alpha_feats*= resolution;
					score_feats = exp ( - feat_dis * feat_dis / (2 * alpha_feats * alpha_feats));
					// score = 0.5 * feat_dis_standard + 0.5 * dis_standard;
					score = score_feats;
					if ( score < - alpha_feats ) 
						score = 1;
					// score = score_dis;
...
// // 将 PCL 点云转换为 Eigen 矩阵
// MatrixXf pointCloudToEigen(const PointCloud& cloud) {
//     int num_points = cloud.points.size();
//     MatrixXf matrix(num_points, 3);
...
// // Welsch 损失函数
// float Welsch1(float x, float c) {
//     return 1.0f - std::exp(-(x / c) / 2.0f);
// }

// // 计算两个点云之间的平方距离矩阵
// MatrixXf compute_sqrdis_map_2(const MatrixXf& points_x, const MatrixXf& points_y) {
//     int pn_x = points_x.rows();
//     int pn_y = points_y.rows();

//     // 扩展点云以计算每个点对的平方距离
//     MatrixXf batch_points_x = points_x.rowwise().replicate(pn_y);
//     MatrixXf batch_points_y = points_y.colwise().replicate(pn_x).transpose();

//     // 计算每个点对的欧氏距离的平方
//     MatrixXf diff = batch_points_x - batch_points_y;
//     MatrixXf dis = diff.rowwise().squaredNorm();
//     return dis;
// }

// // Chamfer 距离计算函数
// double chamfer_dist(const MatrixXf& points_x, const MatrixXf& points_y) {
//     MatrixXf sqrdis = compute_sqrdis_map_2(points_x, points_y);

//     // 每行和每列的最小距离
//     VectorXf dist1 = sqrdis.rowwise().minCoeff();
//     VectorXf dist2 = sqrdis.colwise().minCoeff();

//     // 计算 Chamfer 距离
//     double loss = (dist1.sum() + dist2.sum()) / (points_x.rows() + points_y.rows());
//     return loss;
// }
...
//     return Vector3f::Random().normalized();
// }
// #define Random(x) (rand() % x)  // 重新定义宏
...
//     PointCloudPtr src_pts(new PointCloud);
//     PointCloudPtr des_pts(new PointCloud);
...
//         weight_vec.setOnes(); // 如果不需要加权或者是相同实例，则所有权重为1
//     }
...
//     // 5. 使用变换矩阵对源点云进行变换
//     PointCloudPtr src_trans(new PointCloud);
//     pcl::transformPointCloud(*src_corr_pts, *src_trans, trans);

//     // 6. 将 PCL 点云转换为 Eigen 矩阵，用于 Chamfer Loss 和 Welsch 计算
//     MatrixXf src_points = pointCloudToEigen(*src_trans);
//     MatrixXf des_points = pointCloudToEigen(*des_corr_pts);

//     // 7. 计算 Chamfer Loss
//     double chamfer_loss = chamfer_dist(src_points, des_points);
...
//         double dist = (src_trans->points[i].getVector3fMap() - des_corr_pts->points[i].getVector3fMap()).norm();
//         double w = 1;
//         if (add_overlap) {
...
//     // 10. 结合 Chamfer Loss 和 Welsch 损失
//     double total_score = score - chamfer_loss;  // 可以调整 Chamfer Loss 和 Welsch Loss 对总得分的影响权重
...
double evaluation_trans(vector<Corre_3DMatch>& Match, vector<Corre_3DMatch>& correspondnece, PointCloudPtr& src_corr_pts, PointCloudPtr& des_corr_pts, double weight_thresh, Eigen::Matrix4d& trans, double metric_thresh, const string &metric, float resolution, bool instance_equal) {
	...
	for (int i = 0; i < corr_num; i++)
	{
		...
		if (dist < metric_thresh)
		{
			...
			{
				double capped_dist = std::min(dist, metric_thresh); // 将距离限制在 metric_thresh 以内
				score += capped_dist * w; // 累积误差
			}
		}
	}
...
/**
 * @brief 选择更多的准团（quasi-cliques）以供后续计算
 * 
 * @param Graph 图的邻接矩阵
 * @param cliques 所有团的集合
 * @param correspondence 对应关系
 * @param result 每个节点对应的团信息
 * @param remain 保留的团的索引
 * @param num_node 节点总数
 * @param est_num 预计保留的团的数量（可设为较大的值）
 * @param descriptor 描述符（未使用）
 */
void select_more_quasi_cliques(Eigen::MatrixXf& Graph, igraph_vector_ptr_t* cliques, std::vector<Corre_3DMatch>& correspondence, node_cliques* result, std::vector<int>& remain, int num_node, int est_num, std::string descriptor) {
    // 分配并初始化 vis 数组，用于跟踪每个团被引用的次数
    int num_cliques = igraph_vector_ptr_size(cliques);
    int* vis = new int[num_cliques];
    memset(vis, 0, sizeof(int) * num_cliques);
    ...
    for (int i = 0; i < num_cliques; i++) {
        ...
        // 为每个节点关联这个团，如果该团的权重超过某个阈值
        // 这里可以设定一个较低的阈值，以保留更多的团
        float weight_threshold = 0.1f; // 设定一个较低的权重阈值，根据实际情况调整
        if (weight >= weight_threshold) {
            for (int j = 0; j < length; j++) {
                ...
                #pragma omp critical
                {
                    // 允许每个节点关联多个团，因此不覆盖已有的关联
                    // 可以考虑使用一个容器来存储多个关联的团索引和权重
                    // 这里为了简化，仍然保留最后一个关联的团
                    // 实际应用中可以根据需要进行扩展
                    result[k].clique_index = i;
                    result[k].clique_size = length;
                    result[k].clique_weight = weight;
                }
            }
    ...
    for (int i = 0; i < num_node; i++) {
        ...
        if (vis[result[i].clique_index] > 0) {
            // 不重置 vis[result[i].clique_index]，以允许多个节点关联同一个团
            after_delete.push_back(result[i].clique_index);
        } else {
            result[i].clique_index = -1;
        }
    ...
    // 如果保留的团数量仍然超过 est_num，可以进一步减少，但此处可以选择不限制
    // 以保留更多的团进行后续计算
    // 如果仍需限制，可以注释掉以下代码块
    /*
    if (remain.size() > est_num) {
        ...
        std::sort(clique_score.begin(), clique_score.end(), compare_vote_score);
        for (int i = 0; i < est_num && i < clique_score.size(); i++) {
            after_decline.push_back(clique_score[i].index);
        }
        remain.clear();
        remain = after_decline;
...
void find_largest_quasi_clique(Eigen::MatrixXf& Graph, igraph_vector_ptr_t* cliques, vector<Corre_3DMatch>& correspondence, node_cliques* result, vector<int>& remain, int num_node, int est_num, string descriptor) {
    int* vis = new int[igraph_vector_ptr_size(cliques)];
    memset(vis, 0, igraph_vector_ptr_size(cliques));
    ...
    // reduce the number of quasi-cliques
    if (remain.size() > est_num) {
        vector<int> after_decline;
        vector<Vote> clique_score;
        for (int i = 0; i < num_node; i++) {

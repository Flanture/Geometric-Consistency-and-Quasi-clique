...
typedef struct {
	...
	double score;
	int inlier_weight;
    Eigen::VectorXd src_features; // 新增的源特征向量
    Eigen::VectorXd des_features; // 新增的目标特征向量
}Corre_3DMatch;
typedef struct {
	int PointID;
	float eig1_2;
...
double evaluation_trans_with_weight(vector<Corre_3DMatch>& Match, vector<Corre_3DMatch>& correspondnece, PointCloudPtr& src_corr_pts, PointCloudPtr& des_corr_pts, double weight_thresh, Eigen::Matrix4d& trans, double metric_thresh, const string &metric, float resolution, bool instance_equal);
